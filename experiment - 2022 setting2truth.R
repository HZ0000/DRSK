...
for (i in 1:samplesize) {
  #print(i)
  aa1 <- rgamma(1, shape=20, scale = 1/0.3)
  aa2 <- rgamma(1, shape=20, scale = 1/0.5)
  input_raw <- c(aa1, aa2)
  input_raw <- c(input_raw, rgamma(1, shape=20, scale = 1/0.4))
  input_raw <- c(input_raw, rgamma(1, shape=20, scale = 1/0.2))
  input_raw <- c(input_raw, rgamma(1, shape=20, scale = 1/0.3))
  input_raw <- c(input_raw, rgamma(1, shape=20, scale = 1/1.1))
  input_raw <- c(input_raw, rgamma(1, shape=20, scale = 1/0.2))
  input_raw <- c(input_raw, rgamma(1, shape=20, scale = 1/1.0))
  input_raw <- c(input_raw, rgamma(1, shape=20, scale = 1/0.9))
...
write.csv(arr_in, file = "CF2-truth.csv")
