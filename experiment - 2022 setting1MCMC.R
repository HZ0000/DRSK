...
log_target2 = function(x){
  return(dgamma(x, shape=20, scale = 1/0.7, log=TRUE))
}
log_target3 = function(x){
  return(dgamma(x, shape=20, scale = 1/0.6, log=TRUE))
}
...
log_target6 = function(x){
  return(dgamma(x, shape=20, scale = 1/1.2, log=TRUE))
}
...
log_target10 = function(x){
  return(dgamma(x, shape=20, scale = 1/0.8, log=TRUE))
}
...
write.csv(arr_in, file = "CF1-100mean-MCMC1.csv")
